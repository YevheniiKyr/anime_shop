...
const ReviewItem = observer(({review}) => {
    ...
    useEffect(() => {
        fetchUser(review.user).then(data => setUserComment(data))
    }, [])
        ...
        <Container className={"d-flex"}>
            {
                isEditing ? (
                    <Card.Body style={{marginTop: '2rem', background: '#F5F5F5'}}>
                        <Form>
                            <Form.Label style={{fontSize: '1.5rem'}} column={true}>Відредагуйте коментар</Form.Label>
                            <Form.Group controlId="editContent">
                                <Form.Label column={true}> Коментар </Form.Label>
                                <Form.Control
                                    as="textarea"
                                    style={{resize: "none"}}
                                    rows={3}
                        ...
                        <Card.Body>
                            <ListGroup>
                                <ListGroupItem>
                                    <Row>
                                        <Col xs={8} md={8} lg={8}>
                                            <Container style={{fontSize: '1.5rem'}}>{userComment.email}</Container>
                                        </Col>
                                        <Col xs={4} md={4} lg={4}>
                                            {
                                ...
                                <ListGroupItem className={"d-flex justify-content-center"}>
                                    <RatingAlt rating={review.rating} readOnly={true}/>
                                </ListGroupItem>
                            </ListGroup>
                        </Card.Body>
                    </Card>
